...
for (let j = 0; j < wires.length; j++) {
  let coords = { ...start };
  let steps = 0;
  for (let c = 0; c < wires[j].length; c++) {
    let [dir, ...dist] = wires[j][c].split("");
    dist = +dist.join("");
    ...
    for (let i = 0; i < dist; i++) {
      ...
      steps++;
      if (
        wire[`${coords.x},${coords.y}`] &&
        wire[`${coords.x},${coords.y}`].wire != j + 1
      ) {
        const cross = {
          coords: { ...coords },
          steps: steps + wire[`${coords.x},${coords.y}`].steps
        };
        crosses.push(cross);
      } else {
        wire[`${coords.x},${coords.y}`] = { wire: j + 1, steps };
      }
    }
  }
}

const midDist = crosses.reduce((acc, curr) => {
  const d =
    Math.abs(curr.coords.x - start.x) + Math.abs(curr.coords.y - start.y);
  if (d < acc) {
    return d;
  }
  return acc;
...
const minSteps = Math.min(...crosses.map(c => c.steps));

console.log(midDist, minSteps);
