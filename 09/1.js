...
while (
  ...
  if (commands[i] === 99) {
    ...
    commands[findIndex(commands, +com[0], i + 3, base)] =
      findValue(commands, +com[2], i + 1, base) +
      findValue(commands, +com[1], i + 2, base);
    i += 4;
  } else if (
    commands[i]
      .toString()
    ...
    commands[findIndex(commands, +com[0], i + 1, base)] = 2;
    i += 2;
  } else if (
    commands[i]
      .toString()
    ...
    const com = commands[i].toString().padStart(3, "0");
    console.log(findValue(commands, +com[0], i + 1, base));
    i += 2;
  } else if (
    commands[i]
      .toString()
    ...
    if (findValue(commands, +com[1], i + 1, base) !== 0) {
      i = findValue(commands, +com[0], i + 2, base);
    } else {
      i += 3;
    }
  } else if (
    ...
    if (
      ...
      commands[findIndex(commands, +com[0], i + 3, base)] = 1;
    } else {
      commands[findIndex(commands, +com[0], i + 3, base)] = 0;
    }
    i += 4;
  } else if (
    commands[i]
    ...
    base += findValue(commands, +com[0], i + 1, base);

    i += 2;
  }
}

function findValue(commands, mode, i, base) {
  if (mode === 0) {
    return commands[commands[i]] ? commands[commands[i]] : 0;
  } else if (mode === 1) {
    return commands[i] ? commands[i] : 0;
  } else if (mode === 2) {
    return commands[base + commands[i]] ? commands[base + commands[i]] : 0;
  }
}

function findIndex(commands, mode, i, base) {
  if (mode === 0) {
    return commands[i];
  } else if (mode === 1) {
    return i;
  } else if (mode === 2) {
    return base + commands[i];
  }
}
